...
class ApiClient {
  ...
  Future<dynamic> patch(String path, {Object? body, bool auth = true}) async {
    return _send(auth, () => _dio.patch(path, data: body, options: _opts(auth: auth)));
  }

  Future<dynamic> put(String path, {Object? body, bool auth = true}) async {
    return _send(auth, () => _dio.put(path, data: body, options: _opts(auth: auth)));
  }
...
/// Singleton (même pattern que l'app client).
final api = ApiClient();
